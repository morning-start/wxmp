...
class CustomSpider(TimeRangeSpider):
    def custom_search_logic(self, keywords: list[str]):
        ...
        for keyword in keywords:
            ...
            results.extend(response.arr)
        return results

    def custom_filter_articles(self, articles, min_read_num: int = 100):
        """自定义文章过滤"""
        return [a for a in articles if a.read_num >= min_read_num]
...
class ExtendedAPI(WxMPAPI):
    def custom_api_call(self, custom_params: dict):
        """自定义 API 调用"""
        url = self.domain + "/custom/endpoint"
        params = {"token": self.token, **custom_params}
        res = self.session.get(url, params=params, headers=self.headers, cookies=self.cookies)
        return res.json()
...
result = api.custom_api_call({"key": "value"})
```
